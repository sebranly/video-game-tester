...
The game should naturally introduce more and more concepts as the player progresses. There are a lot of ways to achieve that, some of them are:
...
Whenever possible, it's always a good idea to implement a level editor so that players can create their own levels. It's an amazing plus if they can share them online (or through a copy/paste for less advanced projects that don't implement network interactions).
...
- offer simple controls. For instance, the player should be able to hold a key/mouse click and move to color a column/row of cells at once. A right click should be intuitive and delete the item.
...
I think that the golden rule is to create the level editor at the very beginning of the project and then to use it for every single level. If the process of creating a new level by using the editor feels fastidious, it means that the players won't appreciate the level editor. Usually AAA games are built based on an internal level editor tool. Some of them even got released publicly, like Mario Maker for instance.
...
|:earth_americas: English and French support on the website|:white_check_mark:|:white_check_mark:|
|:earth_americas: English and French support within the game|:x:|:white_check_mark:|
...
|:mouse: Mouse clicks work for selecting the menu (or within the level editor)|:x:|:x:|
|:mouse: The mouse is properly hidden from the screen when completing a level (I'd expect not to see the cursor otherwise it's as annoying as watching a movie with the cursor appearing on top)|:x:|:x:|
|:gem: The menu shows hints about the progression of the player. For instance when on the menu, I'd like `Adventure` to have an icon or a progress bar indicating how far I completed the overall game.|:x:|:x:|
|:signal_strength: Effort into offering an online experience|:white_check_mark:|:white_check_mark:|
...
- :arrow_upper_right: Diagonal moves are possible by holding two keys simultaneously (e.g. this is really useful in level 36)
- :heart_decoration: A real difficult mode with some decent challenge (the player has to really pay attention to their stock of bombs otherwise they can be stuck somewhere with no bombs left nor lives and die quickly like in level 51)
...
- :skull: I was a little bit surprised my bomb wouldn't make me die early in the game. Later on, when trying to complete difficult levels I understood such a gameplay decision
...
- :repeat_one: Pretty repetitive overall, the gameplay and game in general doesn't really evolve apart from the 3 different themes and the new type of enemies appearing (see [this section](https://github.com/sebranly/video-game-tester/blob/master/README.md#evolution)). For instance, it would have been amazing to get access to more power-ups like a star that makes the player invincible for a short period of time (like in Mario) so that the player can run through as many enemies as possible to kill them (like in Pac-Man). Another example would be to offer a special bomb (limited to 1 per level) that has a longer radius of explosion (or even maybe an infinite radius that is still blocked by bricks though). And another example could be a bomb that explodes bricks that cannot usually be destroyed. Playing in the same visual theme for 20 levels becomes a bit boring as well.
- :triangular_ruler: The level editor is so poor I doubt players would want to create more than 3 levels with it (see [this section](https://github.com/sebranly/video-game-tester/blob/master/README.md#level-editor))
- :signal_strength: Online game can really be improved. For instance, it's misleading as it's not clear if this is competitive or co-operative. We were surprised by the fact the diamonds' changes (when player A picks one of them) were not reflected for everyone (player B still sees it, we understood why afterwards but it can be counter-intuitive at the beginning as we can see each other's character). It felt like we were not really playing together because we didn't have a clear impact on each other except forcing one of the players to follow us into the next level. The online game wasn't really lagging in the way that my own character was running smoothly on my own screen but the other player's character's position was really slow to update (~3-4 seconds) despite playing in the same room on the same WiFi (at this point I'd rather just play an offline multiplayer mode with one keyboard). Also, the game froze (black screen) for the host after 4-5 levels and later on, creating/joining a new session was impossible for both of us despite killing/re-launching the app.
- :globe_with_meridians: No mini-map is displayed. That might have been a choice in order not to make the gameplay too easy, which is understandable. However I think that in that case, the mini-map could have been a special item that is hidden (within a destructible block for instance) that the player has to pick in order to activate it and have a global overview of the map/ennemies' position/flag's position as a reward (especially since the last levels tend to be huge and it's hard to navigate through some of them despite their symmetries)
...
- :hocho: A hardcode mode could be good. It could be a mode with a maximum of 3 lives for instance, or a mode that doesn't display any bomb/lives indicator on screen. We can also imagine a level in which enemies keep spawning (with a maximal threshold) until the "generator of enemies" (which is a special block) is destroyed with a bomb by the player. Or it could be a level in which there are no safe zones (i.e. the enemies can move everywhere the player can move).
- :neutral_face: Difficult levels become less interesting after a while once the player realizes that the best technique is to wait on a bomb spawner and hold `Space` key to kill enemies for 2-3 minutes before doing any move. It would be good to introduce more power-ups (a green shell that the player throws against enemies for instance, which bounces on bricks by breaking them) in order to solve this issue (see [this section](https://github.com/sebranly/video-game-tester/blob/master/README.md#evolution)). By making the code open-source, developers could submit a PR introducing such a change (see [this section](https://github.com/sebranly/video-game-tester/blob/master/README.md#open-source)).
- :egg: No real secrets in the game like easter eggs or hidden levels that get unlocked after completion
- :feelsgood: It would be good to have some levels that feel like an adventure in the way that a particular level has a starting point and the player has to reach the ending point by following a specific linear path with some challenges in between. Having symmetrical levels is a good idea but all the 60 levels follow this rule and some diversity would be appreciated.
...
- :busts_in_silhouette: Lots of Pokemon to discover as playable characters
...
- :video_game: Controls are frustrating for the attack move and bouncing on walls to switch direction. It should be handled with two different keys on the keyboard rather than trying to combine everything under one key.
- :snail: Having to wait for a whole "new loop" when we miss a jump is frustrating. In Super Mario Run this is nicely handled by making Mario run a bit on the left before reverting back to running towards the right. This wouldn't fit Super Pokemon Run but something similar needs to be implemented. Additionally, the game speed is so slow it becomes boring.
- :question: The special coin in level 4 cannot be completed because once falling back in the pool the musical note trampoline doesn't enable the player to jump back on the platform on the left (which is too high)
...
- :negative_squared_cross_mark: Getting a high-score is too easy because there is no time restraint so the player just has to loop back into the level to get the missing coins. In Super Mario Run this is not possible because Mario reverts back to moving towards the right.
- :eyes: Some visual bugs/improvements (minor bugs):
  - Toad is still unhappy even when we save him
  - the design of the cage for Toad could be improved
  - water and lava sprites should be animated
  - on the high-score panel, my name appeared on the 3rd slot and all the other slots were empty (except for the early levels)
- :bug: Some major bugs:
  - in the adventure mode after every single game over, the application crashes on Mac (this is not happening for the endless mode)
  - in the endless modes, Medium and Hard difficulties cannot be played but are still available as an option. When launching them, the player dies instantly after spawning into a level full of a new type of Pokemon. It looks like it won't bug in the final version but for now, these options should be locked in the alpha version.
